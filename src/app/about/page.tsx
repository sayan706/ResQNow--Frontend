  ...
  return (
    <div ref={container} className="min-h-screen bg-surface dark:bg-slate-950">
      ...
      <div className="md:ml-64 min-h-screen flex flex-col">
        {/* Top bar with search */}
        <header className="w-full h-16 sticky top-0 z-40 bg-white/90 dark:bg-slate-950/90 backdrop-blur-md flex items-center justify-between border-b border-slate-200 dark:border-slate-800 animate-fade-in-down flex-shrink-0">
          <div className="flex items-center h-full">
            {/* Mobile spacer: matches hamburger width (w-16) */}
            <div className="w-16 md:hidden flex-shrink-0" />
            <div className="hidden md:block w-6 flex-shrink-0" />
            <div className="relative hidden sm:block">
              <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-[18px]">search</span>
              <input
                className="pl-9 pr-4 py-2 bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 outline-none rounded-full text-sm text-slate-700 dark:text-slate-300 focus:ring-2 focus:ring-india-saffron/30 w-56 lg:w-72 transition-all duration-300 placeholder:text-slate-400"
          ...
          <div className="flex items-center gap-2 px-4 h-full">
            <button className="h-9 w-9 flex items-center justify-center text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full transition-colors hover:text-india-saffron duration-200">
              <span className="material-symbols-outlined text-[1.2rem]">notifications</span>
            </button>
            <div className="w-px h-6 bg-slate-200 dark:bg-slate-700 mx-1 hidden sm:block" />
            <div className="flex items-center gap-2">
              <span className="font-headline text-sm font-semibold text-[#022448] dark:text-blue-400 hidden md:inline whitespace-nowrap">
                ResQNow Platform
              </span>
              <img
                alt="User profile"
                className="w-8 h-8 rounded-full object-cover border-2 border-india-saffron/30 hover:border-india-saffron transition-colors duration-200"
                src="https://lh3.googleusercontent.com/aida-public/AB6AXuASAO_BgLlQ64X0F2QVcfVkgvaFjmNtXc-9HOkvkXKBndkFt2SKyyKJ2b85lj7r48RKveBWbbMZtv-I2AqnOpHNTLf1ltRs8T1_rFGsGO4BQT0mKlZW8P_z_icDbbDnNbN-8xwSJhQgRY-k0B7YS72f-BVRCUM5NqiJTmzQXo1ke9sjiOPmzJJ4GTv3CV_IQhxh45ngqMdxSsvaSCDn6twTWc10-gSk9Fx-xULoJxbjJQ3ar5hwORg_xb7TNzo93TEop4ZWNnEdRYvN"
              />
            </div>
          </div>
        </header>
        ...
        <div className="p-4 md:p-8 lg:p-10 max-w-7xl mx-auto w-full space-y-20 page-enter">
          ...
          <section className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
            <div className="lg:col-span-7 space-y-6 hero-elem">
              ...
              <div className="flex flex-wrap items-center gap-4 pt-2 hero-elem">
                ...
                <Link
                  href="/login"
                  className="px-8 py-4 border-2 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 rounded-xl font-bold text-base hover:border-india-saffron hover:text-india-saffron dark:hover:text-india-saffron transition-all duration-300"
                >
                  Sign In
                </Link>
              </div>
          ...
          <footer className="py-12 border-t border-slate-200 dark:border-slate-800">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-10">
              <div className="col-span-1 md:col-span-2 space-y-4">
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400 max-w-sm leading-relaxed">
                  The world's leading predictive intelligence platform for emergency services. Engineered for resilience, designed for action.
                </p>
                <div className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 dark:bg-slate-900 rounded-full w-fit border border-slate-200 dark:border-slate-700">
                  <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 tracking-tight">MADE IN INDIA 🇮🇳</span>
                </div>
              </div>
              <div>
                <h5 className="font-bold text-[#022448] dark:text-white mb-4 text-sm uppercase tracking-wider">Platform</h5>
                <ul className="space-y-2.5 text-sm text-slate-500 dark:text-slate-400">
                  {['Dashboard', 'AI Prediction', 'Fleet Mgmt'].map((item) => (
                    <li key={item}>
                      <a className="hover:text-india-saffron transition-colors duration-200" href="#">{item}</a>
                    </li>
                  ))}
                </ul>
              </div>
              <div>
                <h5 className="font-bold text-[#022448] dark:text-white mb-4 text-sm uppercase tracking-wider">Support</h5>
                <ul className="space-y-2.5 text-sm text-slate-500 dark:text-slate-400">
                  {['Documentation', 'Contact', 'Privacy Policy'].map((item) => (
                    <li key={item}>
                      <a className="hover:text-india-saffron transition-colors duration-200" href="#">{item}</a>
                    </li>
                  ))}
            ...
            <div className="mt-10 pt-6 border-t border-slate-200 dark:border-slate-800 text-center">
              <p className="text-xs text-slate-400">© 2025 ResQNow. All rights reserved. Authorized access only.</p>
            </div>
          </footer>
        </div>
        </main>
